...
# for i in range(11,12):
    fig = plt.figure(i)
    el_tree = ET.parse('../log/traj_log_v' + str(i) + '_planner.xml').getroot()

    with open('../log/' + 'rearanged_' + 'cmmd_log_v' + str(i) + '_planner.yaml', 'r') as document:
    ...
    for segment in el_tree: # over segments
        ...
        for obst in segment[7]:
            ...
            outerCirc = plt.Circle((x, y), .25+.2, color='grey', ls = 'solid', lw=0.5, fill=False, alpha=0.5)
    ...
    fig.savefig('../log/traj_log_v' + str(i) + '_planner.png', bbox_inches='tight', dpi=300)
    fig.savefig('../log/traj_log_v' + str(i) + '_planner.pdf', bbox_inches='tight', dpi=300)
# plt.show()
